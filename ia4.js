...
// Utility to toggle dropdown visibility and ARIA attributes
function toggleDropdown(dropdownBox, triggerElement) {
    const isExpanded = dropdownBox.style.display === 'block';
    dropdownBox.style.display = isExpanded ? 'none' : 'block';
...
function closeDropdown(dropdownBox, triggerElement) {
    dropdownBox.style.display = 'none';
    triggerElement.setAttribute('aria-expanded', 'false');
}

// Attach event handlers to a dropdown trigger and its corresponding dropdown box
function attachDropdownHandlers(triggerId, dropdownId) {
    const triggerElement = document.getElementById(triggerId);
    const dropdownBox = document.getElementById(dropdownId);
    ...
    triggerElement.addEventListener('click', (event) => {
        ...
        toggleDropdown(dropdownBox, triggerElement);
    });

    // Keydown to toggle dropdown with Enter/Space, close with Tab
    triggerElement.addEventListener('keydown', (event) => {
        if (event.key === 'Enter' || event.key === ' ') {
            event.preventDefault(); // Prevent default scroll behavior on space
            toggleDropdown(dropdownBox, triggerElement);
        } else if (event.key === 'Tab') {
            closeDropdown(dropdownBox, triggerElement);
        }
    });
...
// Attach dropdown navigation (keyboard-friendly navigation inside dropdowns)
function handleDropdownNavigation(dropdownSelector) {
    const dropdownItems = document.querySelectorAll(`${dropdownSelector} li`);
    dropdownItems.forEach(item => {
        item.setAttribute('tabindex', '0'); // Make list items focusable

        item.addEventListener('keydown', (event) => {
            const currentIndex = Array.from(dropdownItems).indexOf(event.target);
            let nextIndex = null;
...
// Attach handlers to specific elements
attachDropdownHandlers('box1', 'dropdown-box-cards');
attachDropdownHandlers('box2', 'dropdown-box-languages');

// Enable keyboard navigation inside dropdowns
handleDropdownNavigation('#dropdown-box-cards');
handleDropdownNavigation('#dropdown-box-languages');
